...
(function($){
	var GEllipsify = function(targetElement, options){
		...
		$(this.ellipsis).on({
			click:  (typeof this.options.click === "function" && this.options.click) 
					|| (this.options.preserveText && $.proxy(this.restoreContent,this))
		});
	};
	...
	GEllipsify.prototype = {
		...
		crawler: function(node,level,crawlInfo){
			...
			if(node.nodeType === 3){
				...
				if((crawlInfo.totalChar && this.options.maxChars && crawlInfo.totalChar > this.options.maxChars) ||
					(crawlInfo.totalWord && this.options.maxWords && crawlInfo.totalWord > this.options.maxWords)){
					isDone = true;
					saveSiblings = true;
				}
			} else {
		...
		restoreContent: function(){
			...
			for(;counter<storageLength;counter++){
				...
				parentElement.replaceChild(
					this.nodeStorage[counter].node.firstChild,
		...
		saveSiblings: function(node,crawlInfo){
			...
			if(node.nodeType === 3){
				...
				node.nodeValue = GEllipsify.util.getRemainingString(node.nodeValue,crawlInfo,{chars: this.options.maxChars, words: this.options.maxWords});
				this.textNode = node;
			}
		...
		appendEllipsis: function(ellipsis){
			var style = this.options.appendStyle,
				...
				ellipsisCharLength = 0,
				textNodeLen = this.textNode.nodeValue.length;
	...
	GEllipsify.util.getRemainingString = function(textString,crawlInfo,limit){
		var stringCharLen = GEllipsify.util.countCharacters(textString),
			stringWordLen = GEllipsify.util.countWords(textString),
			aggregatedWordLen = crawlInfo.totalWord,
			aggregatedCharLen = crawlInfo.totalChar,
			wordsLimit = limit.words,
			charsLimit = limit.chars,
			remainingWordString = textString,
			remainingCharString = textString;
			
			
			//had the string reaches the word limit?
			//if not, retain the string
			if(wordsLimit 
					&& aggregatedWordLen > wordsLimit 
					&& (wordsLimit - (aggregatedWordLen - stringWordLen)) >= 0){
			...
			if(charsLimit 
					&& aggregatedCharLen > charsLimit
					&& (charsLimit - (aggregatedCharLen - stringCharLen)) >= 0){
